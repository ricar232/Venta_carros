...
const CATEGORIES = [
  ...
  { type: 'Eléctrico', label: 'Eléctrico', key: 'electric', icon: <path d="M13 3 5 13h5l-1 8 8-10h-5l1-8Z" fill="oklch(0.85 0.13 200)" stroke="none" /> },
];
...
export default function Landing() {
  ...
  return (
    <div style={{ fontFamily: "'Manrope', sans-serif", background: 'oklch(0.16 0.014 265)', color: 'oklch(0.97 0.008 265)', minHeight: '100vh', overflowX: 'hidden', position: 'relative' }}>
      <nav
        style={{
          position: 'fixed',
          top: 0,
          ...
          background: scrolled ? 'oklch(0.14 0.012 265 / 0.75)' : 'transparent',
      ...
      >
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 32px', height: 78, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="veltra-nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: 36 }}>
            <Link to="/catalogo" style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Explorar</Link>
            <a href="#como-funciona" style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Cómo funciona</a>
            <a href="#vender" style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Vender</a>
            <a href="#financiamiento" style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Financiamiento</a>
          </div>
          <div className="veltra-nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            {session.token ? (
              ...
              <Link to="/login" style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600, padding: '10px 4px' }}>Iniciar sesión</Link>
            )}
            <Link
              to="/publicar"
              ...
              style={{
                ...
                padding: '12px 22px',
                borderRadius: 100,
                boxShadow: '0 8px 24px -8px oklch(0.63 0.20 275 / 0.6)',
        ...
        {menuOpen && (
          <div className="veltra-nav-mobile-panel">
            <Link to="/catalogo" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Explorar</Link>
            <a href="#como-funciona" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Cómo funciona</a>
            <a href="#vender" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Vender</a>
            <a href="#financiamiento" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Financiamiento</a>
            {session.token ? (
              <>
                {session.user?.role === 'admin' && (
                  <Link to="/admin" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Admin</Link>
                )}
                <Link to="/mis-compras" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Mis compras</Link>
                <Link to="/mis-anuncios" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Mis anuncios</Link>
              ...
              <Link to="/login" onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>Iniciar sesión</Link>
            )}
            <Link
              to="/publicar"
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 2, zIndex: 60, background: 'oklch(1 0 0 / 0.06)' }}>
        <div style={{ height: '100%', background: 'linear-gradient(90deg, oklch(0.63 0.20 275), oklch(0.72 0.17 200))', width: `${scrollProgress}%`, transition: 'width .1s linear' }} />
      </div>
      ...
      <section onMouseMove={onHeroMouseMove} style={{ position: 'relative', padding: '170px 32px 120px', minHeight: '92vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '100vh', overflow: 'hidden', pointerEvents: 'none' }}>
          <div
            style={{
              ...
              backgroundImage:
                'linear-gradient(oklch(0.72 0.17 200 / 0.16) 1px, transparent 1px), linear-gradient(90deg, oklch(0.72 0.17 200 / 0.16) 1px, transparent 1px)',
          ...
          />
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: '38vh', height: 1, background: 'linear-gradient(90deg, transparent, oklch(0.72 0.17 200 / 0.6), transparent)', animation: 'horizonPulse 3.5s ease-in-out infinite' }} />
          <div style={{ position: 'absolute', top: '16%', left: 0, width: 180, height: 2, borderRadius: 2, background: 'linear-gradient(90deg, transparent, oklch(0.72 0.17 200 / 0.95), transparent)', filter: 'blur(0.5px)', animation: 'streakA 3.2s linear infinite', animationDelay: '.2s' }} />
          <div style={{ position: 'absolute', top: '34%', left: 0, width: 130, height: 2, borderRadius: 2, background: 'linear-gradient(90deg, transparent, oklch(0.63 0.20 275 / 0.85), transparent)', filter: 'blur(0.5px)', animation: 'streakB 4.1s linear infinite', animationDelay: '1.4s' }} />
          <div style={{ position: 'absolute', top: '53%', left: 0, width: 220, height: 2, borderRadius: 2, background: 'linear-gradient(90deg, transparent, oklch(0.72 0.17 200 / 0.8), transparent)', filter: 'blur(0.5px)', animation: 'streakA 3.8s linear infinite', animationDelay: '2.3s' }} />
          <div style={{ position: 'absolute', top: '9%', left: 0, width: 100, height: 1.5, borderRadius: 2, background: 'linear-gradient(90deg, transparent, oklch(0.63 0.20 275 / 0.7), transparent)', filter: 'blur(0.5px)', animation: 'streakB 5s linear infinite', animationDelay: '.8s' }} />
        </div>
        <div
          style={{
            position: 'absolute',
            ...
            height: 640,
            borderRadius: '50%',
            background: 'radial-gradient(circle, oklch(0.63 0.20 275 / 0.35), transparent 70%)',
            top: -160,
            left: -120,
            filter: 'blur(40px)',
        ...
        <div
          style={{
            ...
            height: 560,
            borderRadius: '50%',
            background: 'radial-gradient(circle, oklch(0.72 0.17 200 / 0.28), transparent 70%)',
            bottom: -200,
            right: -100,
            filter: 'blur(40px)',
        ...
        <div className="veltra-hero-grid" style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <h1 className="veltra-hero-title" style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 70, lineHeight: 1.02, margin: '0 0 24px', letterSpacing: '-.01em' }}>
              ...
              <span style={{ position: 'relative', display: 'inline-block' }}>
                <em style={{ fontStyle: 'italic', color: 'oklch(0.72 0.17 200)' }}>perfecto</em>
                <span style={{ position: 'absolute', left: 0, right: 0, bottom: 6, height: 9, background: 'linear-gradient(90deg, oklch(0.63 0.20 275 / 0.35), oklch(0.72 0.17 200 / 0.35))', borderRadius: 100, zIndex: -1 }} />
              </span>
              ,<br />sin sorpresas.
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.6, color: 'oklch(0.72 0.015 265)', maxWidth: 520, margin: '0 0 40px' }}>
              El marketplace donde compradores y vendedores se conectan con confianza: cada anuncio verificado, cada historial revisado.
            </p>

            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', background: 'oklch(0.21 0.016 265 / 0.6)', border: '1px solid oklch(1 0 0 / 0.1)', backdropFilter: 'blur(20px)', padding: 10, borderRadius: 100, maxWidth: 640 }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', inset: -30, background: 'radial-gradient(circle, oklch(0.63 0.20 275 / 0.25), transparent 65%)', filter: 'blur(30px)', animation: 'pulseGlow 5s ease-in-out infinite' }} />
            <div style={{ position: 'relative', borderRadius: 28, overflow: 'hidden', border: '1px solid oklch(1 0 0 / 0.12)', background: 'linear-gradient(135deg, oklch(0.60 0.20 275), oklch(0.55 0.02 265))', boxShadow: '0 40px 100px -20px oklch(0 0 0 / 0.7)' }}>
              ...
              {heroCar && (
                <div style={{ position: 'absolute', bottom: 20, left: 20, right: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                  <div style={{ background: 'oklch(0 0 0 / 0.45)', backdropFilter: 'blur(12px)', borderRadius: 16, padding: '14px 18px' }}>
                    ...
                    <p style={{ margin: '4px 0 0', fontSize: 12.5, color: 'oklch(0.85 0.01 265)' }}>{heroCar.year} · {heroCar.mileageFmt} mi · {heroCar.city}</p>
                  </div>
                  <div style={{ background: 'oklch(0 0 0 / 0.45)', backdropFilter: 'blur(12px)', borderRadius: 16, padding: '14px 18px' }}>
                    <p style={{ margin: 0, fontFamily: "'Instrument Serif', serif", fontStyle: 'italic', fontSize: 22, color: 'oklch(0.78 0.15 200)' }}>{heroCar.priceFmt}</p>
                  </div>
                </div>
              )}
            </div>
      ...
      <section ref={trustRef} style={{ ...trustStyle, padding: '36px 0', borderTop: '1px solid oklch(1 0 0 / 0.07)', borderBottom: '1px solid oklch(1 0 0 / 0.07)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', width: 'max-content', animation: 'marqueeScroll 28s linear infinite', gap: 80, padding: '0 40px' }}>
          {MARQUEE_BRANDS.map((b, i) => (
            <span key={i} style={{ fontFamily: "'Instrument Serif', serif", fontSize: 24, color: 'oklch(0.5 0.015 265)', whiteSpace: 'nowrap' }}>{b}</span>
          ))}
        </div>
      </section>
      ...
      <section ref={statsRef} className="veltra-stats-grid" style={{ ...statsStyle, maxWidth: 1280, margin: '0 auto', padding: '90px 32px', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 24 }}>
        {[
          ...
          <div key={label} style={{ textAlign: 'center' }}>
            <p style={{ margin: 0, fontFamily: "'Instrument Serif', serif", fontSize: 52, color: 'oklch(0.72 0.17 200)' }}>{value}</p>
            <p style={{ margin: '8px 0 0', fontSize: 14, color: 'oklch(0.68 0.015 265)', fontWeight: 600 }}>{label}</p>
          </div>
        ))}
      </section>
      ...
      <section ref={categoriesRef} style={{ ...categoriesStyle, maxWidth: 1280, margin: '0 auto', padding: '40px 32px 100px' }}>
        <span style={{ display: 'inline-flex', fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'oklch(0.72 0.17 200)', marginBottom: 14 }}>● Categorías</span>
        <h2 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 40, margin: '0 0 8px' }}>Explora por categoría</h2>
        <p style={{ fontSize: 16, color: 'oklch(0.68 0.015 265)', margin: '0 0 40px' }}>Encuentra exactamente el tipo de vehículo que buscas</p>
        <div className="veltra-categories-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 16 }}>
          {CATEGORIES.map((cat) => (
            <Link
              ...
              style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '28px 16px', borderRadius: 20, background: 'oklch(0.21 0.016 265 / 0.5)', border: '1px solid oklch(1 0 0 / 0.08)', transition: 'all .3s ease' }}
            >
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg, oklch(0.63 0.20 275 / 0.25), oklch(0.72 0.17 200 / 0.25))', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid oklch(1 0 0 / 0.1)' }}>
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="oklch(0.85 0.13 200)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                  {cat.icon}
                </svg>
              </div>
              <span style={{ fontWeight: 700, fontSize: 14.5 }}>{cat.label}</span>
              <span style={{ fontSize: 12, color: 'oklch(0.6 0.015 265)' }}>{cars.length ? typeCount(cat.type) + ' anuncios' : '—'}</span>
            </Link>
          ))}
          <Link
            to="/catalogo"
          ...
          >
            ...
            <span style={{ fontSize: 12, color: 'oklch(0.6 0.015 265)' }}>—</span>
          </Link>
        </div>
      </section>
      ...
      <section ref={featuredRef} style={{ ...featuredStyle, maxWidth: 1280, margin: '0 auto', padding: '20px 32px 110px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 40 }}>
          <div>
            <span style={{ display: 'inline-flex', fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'oklch(0.72 0.17 200)', marginBottom: 14 }}>● Inventario</span>
            <h2 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 40, margin: '0 0 8px' }}>Recién publicados</h2>
            <p style={{ fontSize: 16, color: 'oklch(0.68 0.015 265)', margin: 0 }}>Seleccionados por su historial y condición</p>
          </div>
          <Link to="/catalogo" style={{ textDecoration: 'none', color: 'oklch(0.72 0.17 200)', fontWeight: 700, fontSize: 14.5 }}>Ver catálogo completo →</Link>
        </div>
        {cars.length > 0 && (
        ...
        {noCarsYet && (
          <div style={{ textAlign: 'center', padding: '90px 20px', border: '1px dashed oklch(1 0 0 / 0.15)', borderRadius: 20 }}>
            ...
            <p style={{ fontSize: 14.5, color: 'oklch(0.65 0.015 265)', margin: 0 }}>Corre <code>npm run seed</code> para poblar el catálogo con datos de ejemplo.</p>
          </div>
        )}
      </section>

      {/* HOW IT WORKS */}
      <section id="como-funciona" ref={howRef} style={{ ...howStyle, padding: '110px 32px', background: 'oklch(0.13 0.012 265)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative' }}>
          <h2 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 40, margin: '0 0 60px', textAlign: 'center' }}>
            <span style={{ display: 'block', fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'oklch(0.72 0.17 200)', fontFamily: "'Manrope', sans-serif", marginBottom: 16 }}>● Proceso</span>
          ...
          <div className="veltra-steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32, position: 'relative' }}>
            ...
            {STEPS.map((step) => (
              <div key={step.n} style={{ textAlign: 'center', position: 'relative' }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg, oklch(0.63 0.20 275), oklch(0.72 0.17 200))', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontFamily: "'Instrument Serif', serif", fontSize: 24, color: 'oklch(0.14 0.012 265)', position: 'relative', zIndex: 1 }}>
                  {step.n}
                </div>
                <h3 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 22, margin: '0 0 10px' }}>{step.title}</h3>
                <p style={{ fontSize: 14.5, color: 'oklch(0.68 0.015 265)', lineHeight: 1.6, maxWidth: 280, margin: '0 auto' }}>{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={whyRef} style={{ ...whyStyle, maxWidth: 1280, margin: '0 auto', padding: '110px 32px' }}>
        <h2 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 40, margin: '0 0 8px', textAlign: 'center' }}>
          <span style={{ display: 'block', fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'oklch(0.72 0.17 200)', fontFamily: "'Manrope', sans-serif", marginBottom: 16 }}>● Confianza</span>
          Por qué comprar en VELTRA
        </h2>
        <p style={{ fontSize: 16, color: 'oklch(0.68 0.015 265)', margin: '0 0 50px', textAlign: 'center' }}>Confianza en cada paso de tu compra</p>
        <div className="veltra-why-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
          {WHY_US.map((item) => (
            <div key={item.title} style={{ padding: '30px 24px', borderRadius: 20, background: 'oklch(0.21 0.016 265 / 0.5)', border: '1px solid oklch(1 0 0 / 0.08)' }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, oklch(0.63 0.20 275 / 0.3), oklch(0.72 0.17 200 / 0.3))', marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="oklch(0.9 0.05 250)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                  {item.icon}
                  {item.icon2}
                </svg>
              </div>
              <h3 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 19, margin: '0 0 8px' }}>{item.title}</h3>
              <p style={{ fontSize: 13.5, color: 'oklch(0.65 0.015 265)', lineHeight: 1.55, margin: 0 }}>{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* CTA */}
      <section id="vender" ref={ctaRef} style={{ ...ctaStyle, margin: '60px 32px', padding: '80px 40px', borderRadius: 32, background: 'linear-gradient(135deg, oklch(0.63 0.20 275), oklch(0.72 0.17 200))', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <h2 className="veltra-cta-title" style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, fontSize: 44, margin: '0 0 16px', color: 'oklch(0.14 0.012 265)' }}>¿Tienes un auto para vender?</h2>
        <p style={{ fontSize: 17, margin: '0 0 32px', color: 'oklch(0.2 0.02 265 / 0.85)' }}>Publica tu anuncio gratis en minutos y llega a miles de compradores verificados.</p>
        <Link
          to="/publicar"
      ...
      <footer id="financiamiento" style={{ maxWidth: 1280, margin: '0 auto', padding: '70px 32px 40px' }}>
        <div className="veltra-footer-grid" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 40, borderBottom: '1px solid oklch(1 0 0 / 0.08)', paddingBottom: 50, marginBottom: 30 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              <span style={{ width: 20, height: 20, borderRadius: 6, background: 'linear-gradient(135deg, oklch(0.63 0.20 275), oklch(0.72 0.17 200))', display: 'inline-block', transform: 'rotate(-8deg)', flexShrink: 0 }} />
              <span style={{ fontFamily: "'Instrument Serif', serif", fontStyle: 'italic', fontSize: 26, lineHeight: 1 }}>VELTRA</span>
            </div>
            <p style={{ fontSize: 13.5, color: 'oklch(0.6 0.015 265)', lineHeight: 1.6, margin: '14px 0 0', maxWidth: 260 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p style={{ fontSize: 12.5, color: 'oklch(0.5 0.015 265)', margin: 0 }}>© 2026 VELTRA. Todos los derechos reservados.</p>
          <div style={{ display: 'flex', gap: 12 }}>
            {['IG', 'FB', 'X'].map((s) => (
              <a key={s} href="#" style={{ width: 34, height: 34, borderRadius: '50%', background: 'oklch(1 0 0 / 0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', color: 'oklch(0.7 0.015 265)', fontSize: 12, fontWeight: 700 }}>
                {s}
              </a>
            ))}
          </div>
...
const footerLinkStyle = { display: 'block', textDecoration: 'none', color: 'oklch(0.65 0.015 265)', fontSize: 13.5, marginBottom: 12 };
