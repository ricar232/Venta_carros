  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 32px', height: 74, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link to={isLogin ? '/registro' : '/login'} style={{ textDecoration: 'none', color: 'oklch(0.9 0.01 265)', fontSize: 14.5, fontWeight: 600 }}>
          {isLogin ? '¿No tienes cuenta? ' : '¿Ya tienes cuenta? '}
          <span style={{ color: 'oklch(0.72 0.17 200)', fontWeight: 700 }}>{isLogin ? 'Regístrate gratis' : 'Inicia sesión'}</span>
        </Link>
      </div>
    </nav>
  );
